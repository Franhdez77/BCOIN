...
The repository currently implements **Sprint 0 — Foundation only**. It contains the web and API
runtime, PostgreSQL/Prisma connectivity, health checks, security defaults, tests, and CI. It does
not yet contain authentication, users, wallets, mining, games, rankings, rewards, or admin
features.
...
5. Validate Prisma and start both applications:
...
database with `docker compose down`; the named volume remains intact.
...
| `RUN_DATABASE_TESTS`                                                 | Opts into the real PostgreSQL integration test; never point it at production data. |
...
The normal test suite does not require a database. With PostgreSQL running, execute the real
Prisma/readiness integration test exactly as CI does:
...
Planned later sprints include secure authentication and sessions, user profiles, the internal BIC
wallet and immutable ledger, 24-hour mining sessions, the football quiz, leaderboards, rewards,
administration, and audit logs. A real blockchain/token, crypto transfers, betting, mobile apps,
and speculative distributed infrastructure remain out of scope.
